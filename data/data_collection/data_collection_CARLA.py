...
def update_plot(p_car, p_goal, car_lead, car_follow, text_list, num, num_sample, frame, control):  
    ...
    text_list[5].set_text('velocity of target vehicle: % .2f m/s' % norm_vector(car_lead.get_velocity()))
...
def main():
    ...
    parser.add_argument('-town', type=int, default=4, help='Town number of data collection (1 ~ 5)')
    parser.add_argument('-start_point', type=int, default=30, help='default starting point (0 ~ 300)')
    ...
    try:
        ...
        transform1 = world.get_map().get_spawn_points()[args.start_point]
        ...
        # add a rgb-camera on ego vehicle
        rgb_list = []
        
        # depth camera
        camera_bp = blueprint_library.find('sensor.camera.depth')
        camera_bp.set_attribute('image_size_x',  str(1200))
        camera_bp.set_attribute('image_size_y',  str(352))
        camera_bp.set_attribute('fov',  str(130.0))
        camera_transform = carla.Transform(carla.Location(x=0, z=2.3))
        camera = world.spawn_actor(camera_bp, camera_transform, attach_to=vehicle_follow)
        actor_list.append(camera)
        print('created %s' % camera.type_id)
        camera.listen(lambda image: image.save_to_disk('output/lidar_cdn/DepthMap/%02d_%04d.png' % (FLAG, frame)))
        ...
        camera_bp_rgb2.set_attribute('image_size_x',  str(1200))
        camera_bp_rgb2.set_attribute('image_size_y',  str(352))
        camera_bp_rgb2.set_attribute('fov',  str(130.0))
        camera_transform_rgb2 = carla.Transform(carla.Location(x=0, z=2.3))
        ...
        camera_rgb2.listen(lambda image: image.save_to_disk('output/RGB1/%02d_%04d.png' % (FLAG, frame)))
        
        # RGB camera 3 for data collection
        camera_bp_rgb3 = blueprint_library.find('sensor.camera.rgb')
        camera_bp_rgb3.set_attribute('image_size_x',  str(1200))
        camera_bp_rgb3.set_attribute('image_size_y',  str(352))
        camera_bp_rgb3.set_attribute('fov',  str(130.0))
        camera_transform_rgb3 = carla.Transform(carla.Location(x=0,y=0.5,z=2.3))
        camera_rgb3 = world.spawn_actor(camera_bp_rgb3, camera_transform_rgb3, attach_to=vehicle_follow)
        actor_list.append(camera_rgb3)
        print('created %s' % camera_rgb3.type_id)
        camera_rgb3.listen(lambda image: image.save_to_disk('output/RGB2/%02d_%04d.png' % (FLAG, frame)))
        
        # LIDAR sensor
        lidar_bp = blueprint_library.find("sensor.lidar.ray_cast")  
        lidar_bp.set_attribute('upper_fov', str(60))
        lidar_bp.set_attribute('lower_fov', str(-30.0))
        lidar_bp.set_attribute('range', str(50.0))
        lidar_bp.set_attribute('points_per_second', str(10000000))
        lidar_transform = carla.Transform(carla.Location(x=0, z=2.3))
        lidar = world.spawn_actor(lidar_bp, lidar_transform, attach_to=vehicle_follow)  
        lidar.listen(lambda image: image.save_to_disk('output/LIDAR/%02d_%04d.ply' % (FLAG, frame)))
        ...
        plt.yticks(np.arange(-500, 509, step=50))
        plt.title('off-trajectory data collection')

        # add text on plot
        text_num = plt.text(-450, 450, '', fontsize=10)
        text_sample = plt.text(-450, 420, '', fontsize=10)
        text_frame = plt.text(-450, 390, '', fontsize=10)
        text_pedal = plt.text(-450, 360, '', fontsize=10)
        text_str = plt.text(-450, 330, '', fontsize=10)
        text_v = plt.text(-450, 300, '', fontsize=10)
        text_ratio = plt.text(-450, 270, '', fontsize=10)
    ...
    # data collection
        for frame in range(1000): 
            ...
            f_data = open('output/lidar_cdn/data.txt', 'a+')
            print('images/%02d_%04d.png' % (FLAG, frame), ref_i[-1][0], ref_i[-1][1], ref_i[-1][2], ref_i[-1][3],
                  state_i[-1][0], state_i[-1][1], state_i[-1][2], state_i[-1][3], throttle, str_angle, file=f_data)
            f_data.close()

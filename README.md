...
## Training
To run the training script, run training.py [training.py](training.py). The resource of depth map and relative transformation can be switched by setting arugments<code>-depth, -relative_transform</code>. Other configurations for training, such as batch size, can be changed in corresponding .json config files.  

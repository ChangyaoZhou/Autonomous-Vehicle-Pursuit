...
# import for model
sys.path.append('..')
from models.networks import MyModel_MLP_transform, MyModel_CNN
...
FIG_SIZE = [8,8]
INPUT_SIZE = [1200,352]
SPAWN_POINTS = {(4, 368): ['xy', 5.5, -6], (4, 280): ['y', 8], 
...
def update_plot(num, p_car, p_goal, car_lead, car_follow, text_list, frame): 
    ...
    text_list[3].set_text('gear of the target vehicle: %d' %car_lead.get_control().gear)
    text_list[4].set_text('gear of the ego vehicle: %d' % car_follow.get_control().gear)
    text_list[5].set_text('throttle of the target vehicle: % .2f' %car_lead.get_control().throttle)
    text_list[6].set_text('throttle of the ego vehicle: % .2f' % car_follow.get_control().throttle)
    car_dis = ((car_lead.get_location().x-car_follow.get_location().x) **2 + 
               (car_lead.get_location().y-car_follow.get_location().y) **2)**0.5
    text_list[7].set_text('car distance: % .2f m' % car_dis)
    text_list[8].set_text('dis/v_ref:% .2f ' % (car_dis/(np.linalg.norm(v_lead_vec)+0.01)))
...
def initialize_parameters():
    global Invasion_target, Collision_target, Invasion_ego, Collision_ego, collision_ids_target, collision_ids_ego
    Invasion_target = 0
    Collision_target = 0
    Invasion_ego = 0
    Collision_ego = 0
    collision_ids_target = []
    collision_ids_ego = []
    print('Invasion_target%2d, Collision_target%2d, Invasion_ego%2d, Collision_ego%2d'%(Invasion_target, Collision_target, Invasion_ego, Collision_ego))
   
  
    
def main():
    parser = argparse.ArgumentParser(description='Input the starting position of both vehicles')
    parser.add_argument('-cnn_path', help='Path to the CNN model to be evaluated with.')
    parser.add_argument('-mlp_path', help='Path to the MLP model to be evaluated with.')
    parser.add_argument('-town', type=int, default=4, help='town number')
    parser.add_argument('-spawn_point', type=int, default=5, help='starting point of target vehicle') 
    parser.add_argument('-impulse_level', type = float, default=0, help ='level of random impulse disturbance')
    parser.add_argument('--all', action='store_true', default=False, help='run all trajectories')
    ...
    for i in range(times):
        ...
        Current_frame = 1000
        if args.all:
            args.town, args.spawn_point = list(SPAWN_POINTS)
            print('Town%2d, spawn point%3d'%(args.town, args.spawn_point))
        try: 
            ...
            vehicle_lead.set_autopilot(True, tm_port) 
            ...
            camera_transform_rgb2 = carla.Transform(carla.Location(z=2.3))
            ...
            camera_rgb2.listen(lambda image: rgb2_list.append(center_resize(image, 350, 600)))
            ...
            text_glead = plt.text(-450, 360, '', fontsize = 8)
            text_gfollow = plt.text(-450, 330, '', fontsize=8) 
            text_tlead = plt.text(100, 360, '', fontsize = 8)
            text_tfollow = plt.text(100, 330, '', fontsize=8) 
            text_dis = plt.text(-450, 300, '', fontsize = 8)
            text_ratio = plt.text(-450, 270, '', fontsize=8) 
            text_list = [text_pt, text_vlead, text_vfollow, text_glead, text_gfollow, text_tlead, text_tfollow, text_dis, text_ratio]
            ...
            mynet_CNN.load_state_dict(torch.load(args.cnn_path, map_location='cpu'))

            mynet_MLP = MyModel_MLP_transform(neurons = [256, 1024, 256])  
            mynet_MLP.load_state_dict(torch.load(args.mlp_path, map_location='cpu')) 
            ...
            for frame in range(1000):
                ...
                if (frame % 40) == 30 or (frame % 40) == 31 or (frame % 40) == 32:          
                ...
                vehicle_follow.apply_control(carla.VehicleControl(throttle=throttle, steer=str_angle, manual_gear_shift=True, gear=vehicle_lead.get_control().gear))
            ...
            f_data = open('./output/evaluation.txt','a+')
            ...
            name_xls = './output/evaluation.xls'
